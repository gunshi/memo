    ...
    def evaluate_agent(
        ...
        while envs.num_envs:
            ...
            for _i in tqdm.tqdm(range(n_steps)):
                ...
                with inference_mode():
                    start_token = (_summary_idx - 1)*self.summary_length if _summary_idx > 0 else 0
                    end_token = _summary_idx*self.summary_length
                    action_data, outputs = agent.actor_critic.act(
                        batch,
                        {
                            'past_key_values': test_recurrent_hidden_states['past_key_values'][..., 1:_i+1, :],
                            'softprompt': test_recurrent_hidden_states['softprompt'][..., start_token:end_token, :]
                        },
                        prev_actions,
                        not_done_masks[:, :_i+1],
                        deterministic=False,
