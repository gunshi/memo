...
from memo.tasks.sensors import OBJ_MAPPING
...
class TransformersRepEpsHabitatEvaluator(Evaluator):
    ...
    def evaluate_agent(
        ...
        VIS_KEYS = set(["head_rgb", "third_rgb", "masked_semantic_sensor"])
        max_scale_semantic_sensor = {"masked_semantic_sensor": 19}
        ...
        if len(config.habitat_baselines.eval.video_option) > 0:
            ...
            rgb_frames: List[List[np.ndarray]] = [
                [
                    observations_to_image(
                        {k: v[env_idx] for k, v in batch.items()}, {}, max_scale_semantic_sensor
                    )
                ]
        ...
        while envs.num_envs:
            ...
            for _i in tqdm.tqdm(range(n_steps)):
                ...
                if 'one_hot_target_sensor' in batch:
                    batch['one_hot_target_sensor'] = batch['one_hot_target_sensor'].float() # .half()
                if 'one_hot_receptacle_sensor' in batch:
                    batch['one_hot_receptacle_sensor'] = batch['one_hot_receptacle_sensor'].float() # .half()
                with inference_mode():
                    start_idx = 0
                    if fifo_eval:
                        start_idx = max(0, _i - 6000)

                    action_data, outputs = agent.actor_critic.act(
                        batch,
                        mem if is_memory else test_recurrent_hidden_states[..., start_idx+1:_i+1, :],
                        prev_actions,
                        not_done_masks[:, start_idx:_i+1],
                        deterministic=False,
                        output_attentions=False
                    )
                    
                    # alternative methods tried out to test the FIFO setting but all were equally bad
                    # this includes getting rid of not datapoints but episodes in case the model is not used to chopped off episodes

                    # start_idx = 0
                    # if fifo_eval:
                    #     # if we are now going beyond decided_thresh steps, we want to clip the start_idx to be decided_thresh steps ago
                    #     # but we also want to take into account episode boundaries and thus choose the clipping value to the most recent episode boundary after decided_thresh steps ago
                    #     if _i > decided_thresh:
                    #         clip_value = start_ep_iter[0]
                    #         while clip_value < _i - decided_thresh:
                    #             clip_value = start_ep_iter.pop(0)
                    #         start_idx = clip_value
                       
                    #     # start_idx = max(0, _i - decided_thresh)
                    #     # print(start_idx, _i)
                    #     # print(test_recurrent_hidden_states[..., start_idx+1:_i+1, :].shape)
                    #     # instead of doing pure FIFO, we actually want to use the first 10 steps and then whatever we would get from FIFO

                    #     mem_tokens_to_use = test_recurrent_hidden_states[..., start_idx+1:_i+1, :]
                    #     dones_to_use = torch.cat((not_done_masks[:, 0:1], not_done_masks[:, start_idx+1:_i+1]), dim=1)
                    # else:
                    #     mem_tokens_to_use = test_recurrent_hidden_states[..., start_idx+1:_i+1, :] if not is_memory else mem
                    #     dones_to_use = not_done_masks[:, start_idx:_i+1]

                    # action_data, outputs = agent.actor_critic.act(
                    #     batch,
                    #     mem if is_memory else mem_tokens_to_use,
                    #     prev_actions,
                    #     dones_to_use,
                    #     deterministic=False,
                    #     output_attentions=False,
                    ...
                    if action_data.should_inserts is None:
                        if is_memory:
                            ...
                            test_recurrent_hidden_states[..., _i+1, :] = (
                        ...
                        for i, should_insert in enumerate(
                            ...
                            if should_update_recurrent_hidden_states:
                                if is_memory:
                                    ...
                                    test_recurrent_hidden_states[:, :, i,..., _i+1, :] = action_data.rnn_hidden_states[i]
                ...
                for i in range(n_envs):
                    ...
                    if len(config.habitat_baselines.eval.video_option) > 0:
                        ...
                        frame = observations_to_image(
                            {k: v[i] for k, v in batch.items()}, disp_info, max_scale_semantic_sensor
                        )
