        ...
        for _bid, batch in enumerate(data_generator):
            ...
            for k, v in batch.items():
                if torch.is_tensor(v) and torch.isnan(v).any():
                    raise Exception(f"NAN detected in {k} batch key.")
            ...
            with nullcontext() \
                ...
                with torch.autocast("cuda", enabled=False):
                    ...
                    if torch.isnan(action_loss).any():
                        raise Exception("Action loss has nan values.")
                    if torch.isnan(value_loss).any():
                        raise Exception("Value loss has nan values.")
                    if torch.isnan(dist_entropy).any():
                        raise Exception("Entropy loss has nan values.")
