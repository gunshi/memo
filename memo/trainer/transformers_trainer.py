...
from memo.default_structured_configs import SPLGeodiscMeasurementConfig, SoftSPLGeodiscMeasurementConfig
...
class TransformersTrainer(PPOTrainer):
    ...
    def _eval_checkpoint(
        ...
        with read_write(config):
            ...
            config.habitat_baselines.vector_env_factory._target_ = "memo.envs.train_env_factory.HabitatVectorEnvFactory"
            ...
            evaluation_config = {
                ...
                'max_num_start_pos': -1,
                'data_path': 'data/datasets/ExtObjNav_HSSD_Diverse/{split}/rearrange_ep_dataset.json.gz' 
            }
        ...
        if config.habitat_baselines.rl.policy.main_agent.transformer_config.model_name=="llamarl":
            evaluator = hydra.utils.instantiate({"_target_": "memo.trainer.TransformersRepEpsHabitatEvaluator"})
            ...
            if config.habitat_baselines.evaluation_type=="default":
                evaluator = hydra.utils.instantiate({"_target_": "memo.trainer.AC_Evaluator"}) 
            ...
            evaluator = hydra.utils.instantiate({"_target_": "memo.trainer.RMT_Evaluator"})
    ...
    def _update_agent(self, loss_from_step=0, last_update=True, percent_envs_update=None, slice_from=0):
        ...
        if torch.isnan(next_value).any():
            raise Exception("NAN in next_value")
