...
class ObjNavEpisodeIterator(Iterator[T]):
    ...
    def _group_scenes(
        ...
        print(f"There are {len(set((x.scene_id, x.object_category) for x in episodes))} scenes and targets.")
        groups = deque([list(x[1]) for x in groupby(episodes, lambda x: (x.scene_id, x.object_category))])
