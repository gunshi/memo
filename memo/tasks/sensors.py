...
class OneHotReceptacleSensor(Sensor):
    ...
    def get_observation(self, *args, **kwargs):
        ...
        if rec_name not in self._all_objs:
            raise ValueError(
                f"Receptacle not found given {rec_name}."
            )
...
from habitat.sims.habitat_simulator.habitat_simulator import HabitatSimSensor, check_sim_obs, HabitatSimSemanticSensor
...
class MaskedSemanticSensor(Sensor):
    ...
    def get_observation(
        ...
        ids = {}
        for obj, id_ in self._task.object_handle2id.items():
            obj_name = obj.split(":")[0].rsplit("_", 1)[0]
            if obj_name in self._all_objs:
                # get the index of the object in the list
                ids[id_] = self._all_objs.index(obj_name) + 1


        obs_mask = np.zeros_like(obs, dtype=np.uint8)
        for id_, obj_list_id in ids.items():
            obs_mask[obs == id_ + self._task._sim.habitat_config.object_ids_start] = obj_list_id
    ...
    def get_observation(
        ...
        for i in range(self.config.n_cells):
            for j in range(self.config.n_cells):
                feats[i*self.config.n_cells + j] = obs_mask[i*STEP:(i+1)*STEP, j*STEP:(j+1)*STEP].mean()
...
class IntegratedPointGoalGPSAndCompassSensorV3(PointGoalSensor):
    ...
    def get_observation(
        ...
        if self.last_goal is None or (self.last_goal != self._task.random_obj_pos).any():
            self.last_goal = self._task.random_obj_pos
            self.noise = (np.random.randn(len(self._task.random_obj_pos)) * self.counter/1_000_000 * self.config.get("std_noise_1m", 0))
...
# @registry.register_sensor(name="SuccessSensor")
# class SucessSensor(PointGoalSensor):