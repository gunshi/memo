...
def is_target_occluded(
    ...
    hits = [x.object_id for x in raycast_results.hits if x.ray_distance < 1 and x.object_id not in ignore_object_ids]
    if ignore_objects:
...
def remove_closest(c, points, task, angle=None):
    ...
    if is_connected(l2_dist, dist, angle_diff=angle_diff):
        return remove_closest(points[closest_end_point_index], [x for i, x in enumerate(points) if i != closest_end_point_index], angle=angle_new, task=task)
    else:
        return points
...
def get_navigation_points(
    ...
    for i in np.linspace(0, 2*np.pi, n):
        for trial_i in range(1, max_num_trials+1):
            ...
            projection = task._sim.pathfinder.snap_point(c+shift, task._sim.largest_island_idx)
            dst = np.linalg.norm(projection - c)
            if dst <= r and np.abs(np.arctan2(*np.asarray(projection - c)[[2, 0]]) - i) < angle_tol/180*np.pi:
                break

        if dst <= r and \
            not is_target_occluded(
                c, 
                projection,
                height,
                task,
...
def get_navigation_points_grid(
    ...
    for k in range(max_num_trials):
        c = np.asarray(c)
        island_y = task._sim.pathfinder.get_random_navigable_point_near(c, 3, island_index=task._sim.largest_island_idx)[1]
        ...
        filtered_points = [(x, y, z) for x, y,z  in zip(xs, ys, zs) if task._sim.is_navigable([x, y, z])]
        output = [
            np.asarray(x) for x in filtered_points 
...
def get_obj_pixel_counts(task, margin=None, strict=True):
    ...
    observations = task._sim._sensor_suite.sensors["head_panoptic"].get_observation(sim_obs)
