...
class NavigationMovementAgentAction(ArticulatedAgentAction):
    ...
    def _move_camera_vertical(self, amount: float):
        ...
        for cam_name in agent_data.articulated_agent.params.cameras:
            *curr_xy, _ = agent_data.articulated_agent.params.cameras[cam_name].cam_look_at_pos
            norm = np.linalg.norm(curr_xy)

            angle = np.arctan2(*curr_xy) - amount/180*np.pi 
            angle = min(max(angle, self.min_angle/180*np.pi), self.max_angle/180*np.pi)
...
class RearrangeLookUpAction(NavigationMovementAgentAction):
    ...
    def __init__(self, *args: Any, **kwargs: Any):
        ...
        for cam_name in agent_data.articulated_agent.params.cameras:
            self.cams_init_pos[cam_name] = list(agent_data.articulated_agent.params.cameras[cam_name].cam_look_at_pos)
    ...
    def reset(self, *args, **kwargs):
        ...
        agent_data = self._sim.get_agent_data(0)
        for cam_name in agent_data.articulated_agent.params.cameras:
            agent_data.articulated_agent.params.cameras[cam_name].cam_look_at_pos = self.cams_init_pos[cam_name].copy()
