...
from memo.policies.transformer_wrappers import (
    MinimalTransformerWrapper,
)

from memo.policies.visual_encoders import Vc1Wrapper
...
class PointNavResNetTransformerPolicy(NetPolicy):
    ...
    def act_inference(
        ...
        **kwargs,
        ...
        features, rnn_hidden_states, _, *output = self.net.forward_inference(
    ...
    def get_value(self, observations, rnn_hidden_states, prev_actions, masks, rnn_build_seq_info=None, full_rnn_state=False):
        features, rnn_hidden_states, _ = self.net(
            observations, rnn_hidden_states, prev_actions, masks, rnn_build_seq_info=rnn_build_seq_info, full_rnn_state=full_rnn_state
        )
    ...
    def get_value_action_prob(self, observations, rnn_hidden_states, actions, prev_actions, masks, rnn_build_seq_info=None, full_rnn_state=False):
        features, rnn_hidden_states, _ = self.net(
            observations, rnn_hidden_states, prev_actions, masks, rnn_build_seq_info=rnn_build_seq_info, full_rnn_state=full_rnn_state
        )
        distribution = self.action_distribution(features)
    ...
    def from_config(
        ...
        **kwargs,
        ...
        return cls(
            ...
            vc1_config=config.habitat_baselines.rl.policy[
                agent_name
            ].vc1_config,
            ...
            append_global_avg_pool=config.habitat_baselines.rl.ppo.get("append_global_avg_pool", False),
        )
    ...
    def evaluate_actions(
        ...
        return_logits=False,
        action_specific_entropy: List[int] = []
    ):
        ...
        # Extract entropy for specific actions
        if len(action_specific_entropy) > 0:
            aux_loss_res['action_specific_entropy'] = []
            for action_idx in action_specific_entropy:
                # Get probabilities for actions 2 and 3
                action_probs = distribution.probs[..., action_idx:action_idx+1]

                # Compute entropy for these specific actions: Entropy = -p*log(p)
                action_entropy = -action_probs * torch.log(action_probs + 1e-10)

                aux_loss_res['action_specific_entropy'].append(action_entropy.mean())
    ...
    def policy_parameters(self) -> Iterable[torch.Tensor]:
        for c in [self.net, self.critic, self.action_distribution]:
...
class ResNetEncoder(nn.Module):
    def __init__(
        ...
        if not self.is_blind:
            spatial_size_h = (
                observation_space.spaces[self.visual_keys[0]].shape[0]
            )
            spatial_size_w = (
                observation_space.spaces[self.visual_keys[0]].shape[1]
            )
    ...
    def forward(self, observations: Dict[str, torch.Tensor]) -> torch.Tensor:  # type: ignore
        ...
        for k in self.visual_keys:
            ...
            if self.key_needs_rescaling[k] is not None:
                obs_k = (
                    obs_k * self.key_needs_rescaling[k]
                )  # normalize
        ...
        output = torch.zeros((x.shape[0], *self.output_shape), dtype=x.dtype, device=x.device)
        if self.gradient_checkpointing and self.training:
            for i in range(0, x.shape[0], 2048):
                output[i:i+2048] = torch.utils.checkpoint.checkpoint(fwd_func, x[i:i+2048], use_reentrant=False)
        else:
            for i in range(0, x.shape[0], 2048):
    ...
    def forward(self, observations: Dict[str, torch.Tensor]) -> torch.Tensor:  # type: ignore
        ...
        if self.head_rgb:
            ...
            head_rgb_observations = torch.stack(
                [self.preprocess(head_rgb_image) for head_rgb_image in head_rgb_observations]
...
class PointNavResNetNet(Net):
    ...
    def __init__(
        ...
        print("using act dim: ", self._n_prev_action)

        self.head_depth_img_dim = 0
        ...
        if 'head_depth' in observation_space.spaces:
            self.head_depth_img_dim = observation_space.spaces['head_depth'].shape[0]
            self.depth_conv_net = nn.Sequential(
                nn.Conv2d(
                ...
                nn.ReLU(inplace=True),
                nn.Flatten(),
                nn.Linear(16 * (self.head_depth_img_dim // 8) * (self.head_depth_img_dim // 8), hidden_size),
                nn.ReLU(inplace=True)
            )
        ...
        if backbone.startswith("vc1"):
            ...
            self.visual_encoder = Vc1Wrapper(use_obs_space, model_id=model_id, vc1_config=vc1_config)
                ...
                if vc1_config.is_2d_output:
                    ...
                    if True:
                        ...
                        compression, output_shape, output_size = create_compression_layer(
                            self.visual_encoder.embd_size, # self.visual_encoder.feats_size, 
                            self.visual_encoder.out_dim
                        )

                        self.visual_fc = nn.Sequential(
                            nn.Unflatten(-1, (self.visual_encoder.embd_size, self.visual_encoder.out_dim, self.visual_encoder.out_dim)), # self.visual_encoder.feats_size
                            compression,
                            nn.Linear(
                    ...
                    self.visual_fc = nn.Sequential(
                        nn.Linear(
                            self.visual_encoder.feats_size, hidden_size
                        ),
                        nn.ReLU(True),
                    )
        ...
        self.state_encoder = MinimalTransformerWrapper(
            (0 if self.is_blind else (self._hidden_size  + (self.visual_encoder.feats_size if self.append_global_avg_pool else 0))) + rnn_input_size,
    ...
    def forward(
        ...
        **kwargs,
        ...
        x = []
        aux_loss_state = {}
        # check that the values of any of the keys in the observations dict are not nan
        # if they are, then raise an exception
        # for k, v in observations.items():
        #     if torch.isnan(v).any():
        #         print(f"Observation {k} has nan values")
                # raise ValueError(f"Observation {k} has nan values")
        ...
        if self.discrete_actions:
            ...
            prev_actions = self.prev_action_embedding(
                act_mask * prev_actions
            )

        x.append(prev_actions)

        if self.head_depth_img_dim > 0 and "head_depth" in observations:
            # head_depth is a 2D image, we need to process it
            head_depth = observations["head_depth"].permute(0, 3, 1, 2)
            head_depth = self.depth_conv_net(head_depth)
            # head_depth is now a 1D tensor of shape [BATCH x hidden_size]
            x.append(head_depth)
        ...
        out, rnn_hidden_states, *output = self.state_encoder(
            out, rnn_hidden_states, masks, rnn_build_seq_info, full_rnn_state=full_rnn_state, **kwargs
        )
        # # print(prof.key_averages().table(sort_by="self_cpu_memory_usage", row_limit=10))
        # torch.cuda.synchronize()
        # self.run_time.append((time.time() - start) * 1000)
        # if len(self.run_time) % 500 == 0:
        #     print(f"average time: {np.mean(self.run_time[-500:])}ms")
        # if len(self.run_time) > 31500:
        #     breakpoint()
    ...
    def forward_inference(
        ...
        **kwargs,
        ...
        out = torch.cat(x, dim=1) + target_embs
        out, rnn_hidden_states, *output = self.state_encoder.forward_inference(
            out, rnn_hidden_states, masks, rnn_build_seq_info, full_rnn_state=full_rnn_state, **kwargs
        )
        aux_loss_state["rnn_output"] = out
        return out, rnn_hidden_states, aux_loss_state, *output