...
class CustomPolicyConfig(PolicyConfig):
    use_aux_future_pred: bool = False
...
class CustomPPOConfig(PPOConfig):
    ...
    aux_future_pred_loss_coeff: float = 0.0
    action_specific_entropy_coeff: List[float] = field(default_factory=list)
    action_specific_entropy: List[int] = field(default_factory=list)

    vis_token_agg: str = "gap" # gap and compression_layer
...
class CustomHabitatBaselinesRLConfig(HabitatBaselinesRLConfig):
    ...
    evaluation_type: str = "default"        # rag, diverse, default
    streaming_setting: str = "default"       # default ??, fifo
