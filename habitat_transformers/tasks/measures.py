...
from habitat_transformers.tasks.utils import get_2d_point, get_obj_pixel_counts, is_target_occluded
import magnum as mn


@registry.register_measure
class PddlTaskSuccess(Measure):
    cls_uuid: str = "custom_pddl_task_success"
    ...
    def sees_object(self, task):
        ...
        for i, (target_ent, target_pos) in enumerate(zip(entities, task.all_obj_pos)):
            if not np.linalg.norm(np.asarray(target_pos - agent_pos)[[0, 2]]) <= task.robot_at_threshold:
                continue
            ...
            if len(snapped_points) == 0:
                ...
                continue
            agent_norms = np.linalg.norm((snapped_points - np.asarray(agent_pos))[:, [0, 2]], axis=1) 
            closest_point = agent_norms.argmin()
            norm = np.linalg.norm((snapped_points[closest_point] - np.asarray(target_pos))[[0, 2]]) 
            if not np.linalg.norm(np.asarray(target_pos - agent_pos)[[0, 2]]) <= max(norm + 0.5, task.robot_at_threshold):
                continue

            object_id = task._sim.get_rigid_object_manager().get_object_id_by_handle(target_ent.name)
            if get_obj_pixel_counts(task, margin=5, strict=False).get(object_id, 0) < self._pixels_threshold:
                continue
    ...
    def update_metric(self, *args, task, **kwargs):
        ...
        if self._metric and self._must_see_object:
            ...
            self._metric = \
                self._metric and \
                    (curr_angle < self._max_angle / 180 * np.pi) and \
                    self.sees_object(task)
...
class NamedNavToObjReward(RearrangeReward):
    ...
    def update_metric(self, *args, episode, task, observations, **kwargs):
        ...
        cur_geodisc_dist = (task.measurements.measures[
                GeoDiscDistance.cls_uuid
        ].get_metric())
        ...
        if self._config.should_reward_turn:
            if cur_geodisc_dist < self._config.turn_reward_dist:
            ...
            if self._cur_angle_dist is None:
                self._cur_angle_dist = angle_dist

            angle_diff = self._cur_angle_dist - angle_dist
            # print(f"Angle diff: {angle_diff}, Last angle dist: {self._cur_angle_dist}, New angle dist: {angle_dist}")
...
class GeoDiscDistance(UsesArticulatedAgentInterface, Measure):
    ...
    def reset_metric(self, *args, episode, task, observations, **kwargs):
        ...
        self._locked_dsts = []
        self.update_metric(*args, task=task, episode=episode, observations=observations, **kwargs)
    ...
    def get_geodisc_distance(self, task, episode):
        ...
        if self._lock_closest_object and not self._locked_dsts:
            closest_object_index = np.searchsorted(task.all_snapped_obj_pos_sizes, path.closest_end_point_index, side="right")
            ...
            self.closest_object_index = np.searchsorted(task.all_snapped_obj_pos_sizes, path.closest_end_point_index, side="right")

        if not (did_find_a_path and not np.isnan(dist) and not np.isinf(dist)):
            # print(f"Failed to calculate distance between {path.requested_start} and {path.requested_ends} in episode {episode.episode_id}. ", did_find_a_path, dist)
            dist = 0
            self.closest_object_index = -1
        elif self._add_point2object_dst:
            dist += np.linalg.norm((task.all_snapped_obj_pos[path.closest_end_point_index] - task.all_obj_pos[self.closest_object_index])[[0, 2]])
...
class L2Distance(UsesArticulatedAgentInterface, Measure):
    ...
    def get_l2_distance(self, task, episode):
        ...
        distances = [np.linalg.norm((np.asarray(p) - np.asarray(agent_pos))[[0, 2]]) for p in task.all_obj_pos]
...
class RotDistToClosestGoal(UsesArticulatedAgentInterface, Measure):
    ...
    def update_metric(self, *args, episode, task, observations, **kwargs):
        ...
        if np.isinf(metric) or np.isnan(metric):
            print(f"Setting angle to 0 because it's {metric}. The target is {targ} and after transformation is {pos}.")
            metric = 0
    ...
    def is_occluded(self, task):
        ...
        object_id = task._sim.get_rigid_object_manager().get_object_id_by_handle(object_name) 
        return is_target_occluded(object_pos, agent_pos, 0, task, ignore_object_ids=set([object_id]), ignore_non_negative=True)
...
class CamRotDistToClosestGoal(UsesArticulatedAgentInterface, Measure):
    ...
    def update_metric(self, *args, episode, task, observations, **kwargs):
        ...
        metric = np.abs(float(angle))
        if np.isinf(metric) or np.isnan(metric):
            print(f"Setting angle to 0 becuase it's {metric}. The target is {targ} and after transformation is {pos}.")
            metric = 0
...
class SPLGeodisc(Measure):
    ...
    def update_metric(
        ...
        self._metric = ep_success * (
            ...
            / max(
                self._start_end_episode_distance, self._agent_episode_distance, 1e-5
            )
        )
