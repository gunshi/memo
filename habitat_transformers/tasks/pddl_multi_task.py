...
from habitat_transformers.tasks.utils import get_navigation_points, get_navigation_points_grid

from collections import defaultdict

def get_pddl(task_config) -> PddlDomain:
    config_path = osp.dirname(
        inspect.getfile(habitat_transformers.default_structured_configs)
    )
...
class CustomRearrangeTask(RearrangeTask):
    def step(self, action: Dict[str, Any], episode: Episode):
        ...
        self._last_success = self.measurements.get_metrics()["custom_pddl_task_success"]
...
class PddlMultiTask(CustomRearrangeTask):
    ...
    def _set_articulated_agent_start(self, agent_idx: int) -> None:
        if self.max_num_start_pos > 0 and len(self.start_pos_cache) == self.max_num_start_pos:
    ...
    def _maybe_cache_start_pos(self, agent_idx: int = 0):
        if self.max_num_start_pos > 0 and len(self.start_pos_cache) < self.max_num_start_pos:
            ...
            self.start_pos_cache.append((articulated_agent.base_pos, articulated_agent.base_rot))
    ...
    def _setup_pddl_entities(self, episode):
        ...
        rec_entity_type = self.pddl.expr_types[SimulatorObjectType.STATIC_RECEPTACLE_ENTITY.value]
    ...
    def get_available_types(self):
        for entity_name, entity_conds in self._sample_entities.items():
            ...
            matches = [
                ...
                if match.expr_type.name not in ["", "any"] + self.to_ignore_types and \
                      (not self.allowed_instances or match.name in self.allowed_instances)
            ]
    ...
    def _load_start_info(self, episode, call_num, no_validation=False, object_type=None):
        ...
        for entity_name, entity_conds in self._sample_entities.items():
            ...
            matches = [
                ...
                for match in matches 
                if match.expr_type.name not in ["", "any"] + self.to_ignore_types and \
                      (not self.allowed_instances or match.name in self.allowed_instances) and \
                        (no_validation or match not in self.to_exclude_entities)
            ]
            ...
            if self.max_n_targets_per_sequence > 0 and len(self.selected_targets) >= self.max_n_targets_per_sequence:
            ...
            if object_type is not None:
                ...
                for name, count in expr_type_names_counts.items():
                    ...
                    if self.target_sampling_strategy == "object_type":
                        ...
                        expr_type_names_p.append(np.sqrt(1/self.success_counts[name]*np.log(sum(self.success_counts.values()))))
                    elif self.target_sampling_strategy == "neg_num_steps":
                        expr_type_names_p.append(-self.accum_num_steps[name] + max(self.accum_num_steps.values()) + 1)
                    else:
                        raise ValueError
                expr_type_names_p = np.asarray(expr_type_names_p)/sum(expr_type_names_p)

                expr_type_name_rnd = np.random.choice(expr_type_names, p=expr_type_names_p)

        self.selected_targets[expr_type_name_rnd] = expr_type_names_counts[expr_type_name_rnd]
        ...
        if self.one_receptacle:
            receptacle = np.random.choice([self.obj_handle2rec_type[x.name] for x in matches])
        ...
        result = self.process_matches(expr_type_name_rnd, object_index, episode.episode_id, no_validation, receptacle, call_num, tuple(self.to_exclude_entities))
        if result:
            self.new_entities, self.all_obj_pos, self.all_snapped_obj_pos, self.all_snapped_obj_pos_sizes, self.new_entities, self._sampled_names = result
        else:
            self.new_entities, self.all_obj_pos, self.all_snapped_obj_pos, self.all_snapped_obj_pos_sizes, self.new_entities, self._sampled_names = {}, [], [], [], {}, []
        ...
        if self._config.get("goal_point_from_snapped", False):
            ...
            self.random_obj_pos = np.asarray(self.all_snapped_obj_pos[start_idx:end_idx][np.random.choice(end_idx - start_idx)])
        else:
            self.random_obj_pos = np.asarray(self.all_obj_pos[np.random.choice(len(self.all_obj_pos))])
        return True
    
    @lru_cache(maxsize=None)
    def process_matches(self, expr_type_name_rnd, object_index, episode_id, no_validation, receptacle, call_num, to_exclude_entities):
        ...
        if self.one_receptacle:
            new_entities = {k: v for k, v in new_entities.items() if self.obj_handle2rec_type[v.name] == receptacle}

        if len(new_entities) >= 10:
            print(f"Ignoring {expr_type_name_rnd} objects in episode {episode_id} because it has more than 9 objects.")
            return False
        ...
        to_keep = [
            len(x) > 0
            for x in all_snapped_obj_pos
        ]

        if self.strict_target_selection and not all(to_keep):
            print(f"Ignoring {expr_type_name_rnd} objects in episode {episode_id} because it has at least one object that is not reachable.")
            return False
        ...
        if not all(to_keep):
            print(f"Removing {len([x for x in to_keep if not x])} targets out of {len(to_keep)} in episode {episode_id}.")

        if not to_keep or not any(to_keep):
            print(f"Object type {expr_type_name_rnd} is not navigable in episode {episode_id}.")
        ...
        return new_entities, all_obj_pos, all_snapped_obj_pos, all_snapped_obj_pos_sizes, new_entities, _sampled_names
    ...
    def set_all_object_ids(self, episode):
        all_objects = [entity for entity in self.pddl.find_entities(self.pddl.expr_types["movable_entity_type"]) if entity.expr_type.name not in ["any", ""]]
        all_objects += [entity for entity in self.pddl.find_entities(self.pddl.expr_types["static_receptacle_entity_type"]) if entity.expr_type.name not in ["any", ""]]
        rom = self._sim.get_rigid_object_manager()
        self.object_handle2id = {entity.name: rom.get_object_id_by_handle(entity.name) for entity in all_objects}
    ...
    def set_all_receptacle_ids(self):
        all_receptacle_handles = set([(x.unique_name.split("|")[0]) for x in find_receptacles(self._sim)])
        rom = self._sim.get_rigid_object_manager()
        self.all_receptacles_ids = [rom.get_object_id_by_handle(name) for name in all_receptacle_handles]

        # TODO: Use config
        df = pd.read_csv("data/scene_datasets/hssd-hab/semantics/objects.csv")
        self.recp2type = {k: v for k, v in df[["id", "main_category"]].values if v in ["bed", "couch", "chair", "tv", "plant", "toilet"]}
    ...
    def reset(self, episode):
        ...
        self.to_exclude_entities = []
        self.is_diff_episode = self.force_is_diff or (self._episode_id != episode.episode_id)
        if self.is_diff_episode:
