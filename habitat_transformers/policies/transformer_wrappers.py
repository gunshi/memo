...
from habitat_transformers.policies.llamarl.configuration_llamarl import (
    LlamaRLConfig,
)
from habitat_transformers.policies.llamarl.modeling_llamarl import (
    LlamaRLModel,
    AutocompLlamaRLModel,
    NewAutocompLlamaRLModel,
    NewRopeAutocompLlamaRLModel,
    DetachOldAutocompLlamaRLModel,
    FullDetachOldAutocompLlamaRLModel,
    DetachNewAutocompLlamaRLModel,
    AdaptiveAutocompLlamaRLModel,
    RMTLlamaRLModel,
    AutoCacheLlamaRLModel,
)
# from transformers import TransfoXLModel, TransfoXLConfig
from transformers import TransfoXLConfig
from habitat_transformers.policies.transformerxl.modeling_transformerxl import TransfoXLModel
import ipdb
...
class MinimalTransformerWrapper(nn.Module):
    def __init__(self, input_size: int, config):
        ...
        self.dont_randomise_segments = config.get("dont_randomise_segments", False)
        ...
        print('feats proj weight: ', self.feats_proj.weight.shape)
        ...
        if self.model_name == "gpt":
        ...
        elif self.model_name == "autocompllamarl":
            self.hf_config = LlamaRLConfig(
                hidden_size=self.n_embed,
                intermediate_size=self.n_mlp_hidden,
                num_hidden_layers=self.n_layers,
                ...
                num_sink_tokens=self.num_sink_tokens,
                sequence_embed_type=self.sequence_embed_type,
                accumulate_summary=self.accumulate_summary,
                summary_length=self.summary_length,
                segment_length=self.segment_length,
                detach_prob=self.detach_prob,
                dont_randomise_segments=self.dont_randomise_segments
            )

            self.model = AutocompLlamaRLModel(self.hf_config)
        elif self.model_name == "autocachellamarl":
            self.hf_config = LlamaRLConfig(
                hidden_size=self.n_embed,
                intermediate_size=self.n_mlp_hidden,
                num_hidden_layers=self.n_layers,
            ...
            self.model = AutoCacheLlamaRLModel(self.hf_config)

        elif self.model_name == "rmtllamarl":
            self.hf_config = LlamaRLConfig(
                hidden_size=self.n_embed,
                intermediate_size=self.n_mlp_hidden,
                num_hidden_layers=self.n_layers,
            ...
            self.model = RMTLlamaRLModel(self.hf_config)
    ...
    def stack_past_key_values(self, past_key_values, last_step=False):
        if past_key_values is None:
            return None
        softprompt_key_values = None
        if isinstance(past_key_values, dict):
            past_key_values_cache = past_key_values.copy()
            past_key_values = past_key_values_cache["past_key_values"]
            softprompt_key_values = past_key_values_cache.get("softprompt", None)
            if past_key_values is None:
        ...
        if last_step:
            past_key_values = torch.stack(
                [torch.stack([y[..., -1, :] for y in x]) for x in past_key_values]              ## might be fetching summary token generation cache when summary gets made but this is not a problematic bug
            )
            if softprompt_key_values is not None:
                softprompt_key_values = torch.stack(
                    [torch.stack([y[..., -self.summary_length:, :] for y in x]) for x in softprompt_key_values]
                )
        else:
            try:
                if past_key_values_cache is not None:
                    num_segments = len(past_key_values)//self.n_layers
                    results = []
                    for seg_num in range(num_segments):
                        res = torch.stack([torch.stack(x) for x in past_key_values[seg_num * self.n_layers:(seg_num+1)*self.n_layers]])
                        results.append(res)
                    past_key_values = torch.cat(results, dim=4)
                    if softprompt_key_values is not None:
                        softprompt_key_values = torch.stack([torch.stack(x) for x in softprompt_key_values])
                else:
                    past_key_values = torch.stack(
                        [torch.stack(x) for x in past_key_values]
                    )
                    if softprompt_key_values is not None:
                        softprompt_key_values = torch.stack(
                            [torch.stack(x) for x in softprompt_key_values]
                        )

            except:
                ipdb.set_trace()

        if isinstance(past_key_values_cache, dict):
            past_key_values_cache["past_key_values"] = past_key_values
            if softprompt_key_values is not None:
                past_key_values_cache["softprompt"] = softprompt_key_values
            return past_key_values_cache
    ...
    def forward(self, feats, rnn_hidden_states, masks, rnn_build_seq_info, full_rnn_state=False, **kwargs):
        if rnn_build_seq_info is None:
            hidden_states_shape = rnn_hidden_states["past_key_values"].shape if isinstance(rnn_hidden_states, dict) else rnn_hidden_states.shape
            past_key_values = rnn_hidden_states if np.prod(hidden_states_shape) > 0 else None
            n_envs = hidden_states_shape[2]
            seq_len = 1
            ...
            if self.model_name == "transformerxl":
                ...
                past_key_values = past_key_values.squeeze(-1).unflatten(0, (self.num_recurrent_layers, self.memory_size)).float()
                use_cache = False
            else:
                past_key_values = None
        ...
        if kwargs:
            return feats, self.stack_past_key_values(output.past_key_values, last_step=self.model_name != "transformerxl" and not full_rnn_state), output
        else:
            return feats, self.stack_past_key_values(output.past_key_values, last_step=self.model_name != "transformerxl" and not full_rnn_state)


    def forward_inference(self, feats, rnn_hidden_states, masks, rnn_build_seq_info, full_rnn_state=False, **kwargs):
        if rnn_build_seq_info is None:
            hidden_states_shape = rnn_hidden_states["past_key_values"].shape if isinstance(rnn_hidden_states, dict) else rnn_hidden_states.shape
            past_key_values = rnn_hidden_states if np.prod(hidden_states_shape) > 0 else None
            n_envs = hidden_states_shape[2]
