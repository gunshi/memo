...
class ACTransformerRolloutStorage(RolloutStorage):
        ...
        if next_recurrent_hidden_states is not None:
            ...
            try:
                if next_recurrent_hidden_states.get("softprompt") is not None:
                    # print("writing summaries at step: ", self.current_summary_idxs[buffer_index])
                    self.summary_vectors[:, :, env_slice, :, self.current_summary_idxs[buffer_index]: self.current_summary_idxs[buffer_index]+self.summary_length] = next_recurrent_hidden_states["softprompt"]  # is it reshaped appropriately? need to find the correct set of 50 vecs
            except:
                ipdb.set_trace()
    ...
    def init_summary_vectors(self):
        with torch.inference_mode(False):
            ...
            # self.summary_vectors[:] = torch.nan
