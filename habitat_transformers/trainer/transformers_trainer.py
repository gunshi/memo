...
from habitat_transformers.default_structured_configs import SPLGeodiscMeasurementConfig, SoftSPLGeodiscMeasurementConfig
...
class TransformersTrainer(PPOTrainer):
    ...
    def _eval_checkpoint(
        ...
        with read_write(config):
            ...
            config.habitat_baselines.num_environments = 22 if config.habitat_baselines.eval.split=="train" else 12
            config.habitat.seed = config.habitat.seed + config.habitat_baselines.num_environments * world_rank
            ...
            config.habitat_baselines.vector_env_factory._target_ = "habitat_transformers.envs.train_env_factory.HabitatVectorEnvFactory"
            ...
            evaluation_config = {
                ...
                'max_num_start_pos': -1,
                'data_path': "/srv/flash1/kyadav32/datasets/habitat_datasets/datasets/hssd_11100_samples_37_scenes_30objmax/sampled111episodes.json.gz" if config.habitat_baselines.eval.split=="train" else
                    'data/datasets/hssd_10epsPerScene_clean/rearrange/{split}/rearrange_ep_dataset.json.gz' 
            }
            print(f"Using data from {evaluation_config['data_path']}")

            evaluation_config.update(config.habitat_baselines.get("evaluation_config", {}))
            if evaluation_config["data_path"]:
                config.habitat.dataset.data_path=evaluation_config["data_path"]
            N_STEPS = evaluation_config["n_steps"]
            N_DEMOS = evaluation_config["n_demos"]
            config.habitat.task.fix_target_same_episode = evaluation_config["fix_target_same_episode"]
            config.habitat.task.max_num_start_pos = evaluation_config["max_num_start_pos"]
            MAX_N_EPS = evaluation_config.get("max_n_eps", -1)

        with read_write(self.config):
            self.config.habitat_baselines.eval.evals_per_ep = 1
        ...
        # choose evaluator based on whether transformer model is autocomp or not
        if config.habitat_baselines.rl.policy.main_agent.transformer_config.model_name=="llamarl":
            evaluator = hydra.utils.instantiate({"_target_": "habitat_transformers.trainer.TransformersRepEpsHabitatEvaluator"})
        elif config.habitat_baselines.rl.policy.main_agent.transformer_config.model_name=="autocompllamarl" or config.habitat_baselines.rl.policy.main_agent.transformer_config.model_name=="autocachellamarl":
            if config.habitat_baselines.evaluation_type=="default":
                evaluator = hydra.utils.instantiate({"_target_": "habitat_transformers.trainer.AC_Evaluator"}) 
        elif config.habitat_baselines.rl.policy.main_agent.transformer_config.model_name=="rmtllamarl":
            assert config.habitat_baselines.evaluation_type=="default"
            evaluator = hydra.utils.instantiate({"_target_": "habitat_transformers.trainer.RMT_Evaluator"})

        print(config.habitat_baselines.rl.policy.main_agent.transformer_config.model_name)
        ...
        evaluator.evaluate_agent(
            ...
            is_memory=config.habitat_baselines.rl.policy.main_agent.transformer_config.model_name=="transformerxl",
            fifo_eval=config.habitat_baselines.streaming_setting=="fifo",
        )
    ...
    def _init_train(self, resume_state=None):
        ...
        if self._is_distributed:
            ...
            with read_write(self.config):
                ...
                self.config.habitat.simulator.habitat_sim_v0.gpu_device_id = (
                    local_rank % (torch.cuda.device_count() - torch.distributed.get_world_size()) + torch.distributed.get_world_size() 
                    if self.config.habitat_baselines.separate_envs_and_policy 
                    else local_rank
                )
        ...
        # self.poses_saving = []
        # self.poses_saving.append(batch['gps'].cpu().numpy())
    ...
    def _reset_envs_custom(self):
        ...
        # print(batch['gps'])
        # self.poses_saving.append(batch['gps'].cpu().numpy())
        # # also dump this to a file
        # with open("poses.npy", "wb") as f:
        #     np.save(f, np.concatenate(self.poses_saving, axis=0))
        # print("Saved poses to poses.npy, shape: ", np.concatenate(self.poses_saving, axis=0).shape)
    ...
    def _update_agent(self, loss_from_step=0, last_update=True, percent_envs_update=None, slice_from=0):
        with inference_mode():
        ...
        # add a check to see if any of the values are nan
        if torch.isnan(next_value).any():
            print("NAN in next_value")
            ipdb.set_trace()
    ...
    def train(self) -> None:
        ...
        if self.config.habitat_baselines.rl.ppo.full_updates_per_rollout > 0:
            assert self.config.habitat_baselines.rl.ppo.updates_per_rollout % self.config.habitat_baselines.rl.ppo.full_updates_per_rollout == 0
            RATIO_P2F_UPDATES = self.config.habitat_baselines.rl.ppo.updates_per_rollout // self.config.habitat_baselines.rl.ppo.full_updates_per_rollout
        else:
            RATIO_P2F_UPDATES = 0

        UPDATE_EVERY_STEPS = self.config.habitat_baselines.rl.ppo.num_steps // self.config.habitat_baselines.rl.ppo.updates_per_rollout
        ...
        if resume_state is not None:
            ...
            # )
            logger.info("Last checkpoint percent: {} - {}".format(self._last_checkpoint_percent, self.percent_done()))
            self._last_checkpoint_percent = min(self._last_checkpoint_percent, self.percent_done())
            logger.info("Resuming from previous checkpoint")
            logger.info("Total num steps: {}/{} - {}".format(self.num_steps_done, self.config.habitat_baselines.total_num_steps, self.percent_done()))
            logger.info("[Override] Last checkpoint percent: {} - {}".format(self._last_checkpoint_percent, self.percent_done()))
            resume_run_id = requeue_stats.get("run_id", None)

        if self.config.habitat_baselines.rl.ppo.get("init_checkpoint", "") and resume_state is None:
            print(f"loading from {self.config.habitat_baselines.rl.ppo.init_checkpoint}...")
            resume_state_ = load_resume_state(self.config.habitat_baselines.rl.ppo.init_checkpoint)
            self._agent.load_state_dict(resume_state_, strict=False)
        ...
        with (
            ...
            while not self.is_done():
                ...
                with g_timer.avg_time("trainer.rollout_collect"):
                    ...
                    for step in range(self._ppo_cfg.num_steps):
                        ...
                        if (step + 1) % UPDATE_EVERY_STEPS == 0 and not is_last_step:
                            IS_FULL_UPDATE = RATIO_P2F_UPDATES != 0 and (step + 1) % (RATIO_P2F_UPDATES * UPDATE_EVERY_STEPS) == 0
                            update_from = (
                                0
                                if IS_FULL_UPDATE or self._ppo_cfg.slice_in_partial_update
                                else last_update_step
                            )
                            slice_from = (
                                0
                                if IS_FULL_UPDATE or not self._ppo_cfg.slice_in_partial_update
                                else last_update_step
                            )
                            ...
                            self._agent.eval()
                            last_update_step = self._agent._rollouts.current_rollout_step_idx

                            if self._ppo_cfg.shuffle_old_episodes:
                                self._agent.rollouts.shuffle_episodes()

                            if self._ppo_cfg.update_stale_kv or self._ppo_cfg.update_stale_values or self._ppo_cfg.update_stale_action_probs:
                                with torch.inference_mode():
                                    for i in range(self.envs.num_envs):
                                        batch = self._agent._rollouts.get_context_step(env_id=i, n_steps=last_update_step)
                                        value_preds, action_log_probs, recurrent_hidden_states = self._agent._actor_critic.get_value_action_prob(
                                            batch["observations"], 
                                            None, 
                                            batch["actions"], 
                                            batch["prev_actions"], 
                                        ...
                                        action_log_probs = action_log_probs.unflatten(0, tuple(batch["rnn_build_seq_info"]["dims"]))
                                        ...
                                        self._agent._rollouts.update_context_data(value_preds, action_log_probs, recurrent_hidden_states, env_id=i, n_steps=last_update_step)
                ...
                losses = self._update_agent(0 if self._ppo_cfg.full_updates_per_rollout > 0 else last_update_step, last_update=True)

                self._agent.eval()
                if self._agent._rollouts.context_length > 0 and (self._ppo_cfg.update_stale_kv or self._ppo_cfg.update_stale_values or self._ppo_cfg.update_stale_action_probs):
                    with torch.inference_mode():
                        for i in range(self.envs.num_envs):
                            ...
                            self._agent._rollouts.update_context_data(value_preds, action_log_probs, recurrent_hidden_states, env_id=i)
                ...
                if self.config.habitat_baselines.call_after_update_env:
                    if self._ppo_cfg.shift_scene_every <= 0:
                        ...
                        shift_every_n_rollouts = np.ceil(self._ppo_cfg.shift_scene_every/self._ppo_cfg.num_steps)
                        to_switch = int(np.ceil(self.envs.num_envs * (rollouts_count/shift_every_n_rollouts)))
                ...
                if self._ppo_cfg.get("force_env_reset_every", -1) > 0:
                    force_reset_n_rollouts = np.ceil(self._ppo_cfg.force_env_reset_every/self._ppo_cfg.num_steps)
                else:
                    force_reset_n_rollouts = -1

                if self.config.habitat_baselines.reset_envs_after_update or (force_reset_n_rollouts > 0 and (rollouts_count + 1) % force_reset_n_rollouts == 0):
