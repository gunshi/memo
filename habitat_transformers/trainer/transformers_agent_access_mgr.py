...
class TransformerSingleAgentAccessMgr(SingleAgentAccessMgr):
    def _init_policy_and_updater(self, lr_schedule_fn, resume_state):
        ...
        if self._updater.optimizer is None:
            ...
            if self._ppo_cfg.warmup:
                scheds.append(
                    optim.lr_scheduler.LinearLR(self._updater.optimizer, start_factor=self._ppo_cfg.warmup_start_factor, total_iters=self._ppo_cfg.warmup_total_iters, end_factor=self._ppo_cfg.warmup_end_factor)
                )
                milestones.append(self._ppo_cfg.warmup_total_iters)


            if self._ppo_cfg.lr_scheduler == "cosine_decay":
                scheds.append(optim.lr_scheduler.CosineAnnealingLR(self._updater.optimizer, T_max=self._ppo_cfg.lrsched_T_max, eta_min=self._ppo_cfg.lrsched_eta_min))
            elif self._ppo_cfg.lr_scheduler == "cosine_annealing_warm_restarts":
                scheds.append(optim.lr_scheduler.CosineAnnealingWarmRestarts(self._updater.optimizer, T_0=self._ppo_cfg.lrsched_T_0, eta_min=self._ppo_cfg.lrsched_eta_min))
            ...
            if scheds and len(scheds) > 1:
                self._lr_scheduler = CustomSequentialLR(self._updater.optimizer, scheds, milestones)
            elif scheds:
    ...
    def _create_storage(
        ...
        dtype = torch.float16 if config.habitat_baselines.rl.ppo.storage_low_precision else torch.float32
        separate_rollout_and_policy = config.habitat_baselines.separate_rollout_and_policy
        ...
        rollouts = baseline_registry.get_storage(
            ...
            calc_cross_eps_returns=getattr(config.habitat_baselines.rl.ppo, "calc_cross_eps_returns", False),
        )
        return rollouts
