...
class MinimalTransformerPPO(PPO):
    ...
    def __init__(
        ...
        action_specific_entropy_coeff: List[float] = [],
    ) -> None:
        ...
        self.action_specific_entropy = np.array(action_specific_entropy)
        self.action_specific_entropy_coeff = np.array(action_specific_entropy_coeff)
    ...
    def get_advantages(self, rollouts: RolloutStorage, slice_from=0) -> Tensor:
        advantages = (
            rollouts.buffers["returns"].to(rollouts.dst_device)  # type: ignore
            - rollouts.buffers["value_preds"].to(rollouts.dst_device)
        )
        if not self.use_normalized_advantage:
            return advantages

        var, mean = self._compute_var_mean(
            advantages[:, slice_from:rollouts.current_rollout_step_idx][torch.isfinite(advantages[:, slice_from:rollouts.current_rollout_step_idx])]
        )
    ...
    def from_config(cls, actor_critic: NetPolicy, config):
        ...
        obj = cls(
            ...
            action_specific_entropy=config.get("action_specific_entropy", []),
            action_specific_entropy_coeff=config.get("action_specific_entropy_coeff", []),
        )
        return obj
    ...
    def update(
        ...
        for epoch in range(self.ppo_epoch):
            ...
            data_generator = rollouts.data_generator(
                advantages, self.num_mini_batch, percent_envs_update, slice_from=slice_from
            )
    ...
    def _create_optimizer(self, lr, eps):
        ...
        if len(params) > 0:
            if self.optimizer_name.lower() == "adamw":
                ...
                decay_parameters = get_parameter_names(self, [nn.LayerNorm, type(self.actor_critic.critic)])
                decay_parameters = [name for name in decay_parameters if "bias" not in name]
                vis_factor = 1
                params = [
                    {
                        "params": [p for n, p in self.named_parameters() if n in decay_parameters and "visual" in n and p.requires_grad],
                        "weight_decay": self.adamw_weight_decay,
                        "lr": lr * vis_factor
                    },
                    {
                        "params": [p for n, p in self.named_parameters() if n in decay_parameters and "visual" not in n and p.requires_grad],
                        "weight_decay": self.adamw_weight_decay,
                        "lr": lr
                    },
                    {
                        "params": [p for n, p in self.named_parameters() if n not in decay_parameters and "visual" in n and p.requires_grad],
                        "weight_decay": 0,
                        "lr": lr * vis_factor
                    },
                    {
                        "params": [p for n, p in self.named_parameters() if n not in decay_parameters and "visual" not in n and p.requires_grad],
                        "weight_decay": 0,
                        "lr": lr
                    },
                ]
    ...
    def _update_from_data_generator(
        ...
        self._set_grads_to_none()
        for _bid, batch in enumerate(data_generator):
            # check if any of the batch keys have nan values, if they are tensors
            for k, v in batch.items():
                if torch.is_tensor(v) and torch.isnan(v).any():
                    print(f"NAN detected in {k} batch key.")
                    ipdb.set_trace()

            with nullcontext() \
                if (not hasattr(self, "_evaluate_actions_wrapper") or (_bid + 1) % self.grad_accum_mini_batches == 0) \
                ...
                with torch.autocast("cuda", enabled=False):
                    ...
                    (
                        ...
                        batch["actions"],
                        rnn_build_seq_info,
                        return_logits=torch.tensor(False),
                        action_specific_entropy=torch.tensor(self.action_specific_entropy),
                    )
                    ...
                    # aux future loss here
                    if self.actor_critic.use_aux_future_pred:                                                                            ## how to check this
                        aux_loss = self.actor_critic.future_pred_module.calc_loss(batch, recurrent_hidden_states)           ## check
                    ...
                    # we have been getting nan error after a few updates, so check for nan in each of the losses
                    if torch.isnan(action_loss).any():
                        print("Action loss has nan values.")
                        print("Action loss: ", action_loss)
                        ipdb.set_trace()
                    if torch.isnan(value_loss).any():
                        print("Value loss has nan values.")
                        print("Value loss: ", value_loss)
                    if torch.isnan(dist_entropy).any():
                        print("Entropy loss has nan values.")
                    ...
                    if isinstance(self.entropy_coef, float):
                        ...
                        if "action_specific_entropy" in aux_loss_res:
                            for i, loss in enumerate(aux_loss_res["action_specific_entropy"]):
                                all_losses.append(-self.action_specific_entropy_coeff[i] * loss)
                            del aux_loss_res["action_specific_entropy"]
                    else:
                        all_losses.append(
                ...
                if (_bid + 1) % self.grad_accum_mini_batches == 0:
                    ...
                    #         print(f"NAN detected in {name} parameter.")
                    #         any_param_nan = True
                            # ipdb.set_trace()
                    # if not any_param_nan:
                    #     print("No NAN detected in parameters.")
        ...
        with inference_mode():
            ...
            # actions = batch["actions"].flatten()  # Flatten in case they are batched
            # action_counts = torch.bincount(actions.int(), minlength=6)  # Ensure all actions (0-3) are counted
            # action_distribution = action_counts.float() / actions.numel()  # Normalize

            # # just add keys for each action in action_distribution
            # for i, action_prob in enumerate(action_distribution):
            #     learner_metrics[f"action_{i}"].append(action_prob)
